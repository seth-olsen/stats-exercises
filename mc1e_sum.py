...
outname_bar = 'sum' + outname + '_bar.txt'
outname_stats = 'sum' + outname + '_stats.csv'
...
np.savetxt(outname_stats, integrated_stats, delimiter=',')

threshold = np.quantile(integrated_stats, 1 - alpha)

f = open(outname_bar, 'w')
f.write('TEST: (1/N) * sum(n) |z|^2\n')
f.write('N  =  ' + str(N) + '\n\n')
f.write('tau  =  ' + str(tau) + '\n\n')
